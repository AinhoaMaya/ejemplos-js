(async () => {
  ...
  try {
    ...
    const temperatures = Object.keys(data).map(month => {
      const years = Object.keys(data[month])
      const firstYear = years[33]

      let media = data[month][firstYear].media
      return media
    })

    const variations = temperatures.map((temperature, index) => {
      if(index === 0) return 0
      return temperature - temperatures[index - 1]
    })
